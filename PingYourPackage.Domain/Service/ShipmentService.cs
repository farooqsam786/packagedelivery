...
namespace PingYourPackage.Domain.Service {
    ...
    public class ShipmentService : IShipmentService {
        ...
        public PaginatedList<Shipment> GetShipments(int pageIndex, int pageSize, Guid affiliateKey) {
            ...
            var shipments = _shipmentRepository.GetShipmentsByAffiliateKey(affiliateKey).OrderBy(x => x.CreatedOn).ToPaginatedList(pageIndex, pageSize);
        ...
        */
        public OperationResult<ShipmentState> AddShipmentState(Guid shipmentKey, PingYourPackage.Domain.Entities.ShipmentState.ShipmentStatus status) {

            if (!IsShipmentStateInsertable(shipmentKey)) {
            ...
            return new OperationResult<ShipmentState>(true) { 
                Entity = (ShipmentState)shipmentState
            };
        }

        private object InsertShipmentState(Guid shipmentKey, ShipmentState.ShipmentStatus status)
        {
            throw new NotImplementedException();
        }

        private bool IsShipmentStateInsertable(Guid shipmentKey)
        {
            throw new NotImplementedException();
        }
        ...
        private ShipmentState InsertFirstShipmentState(Guid ShipmentKey) {
            ...
            return InsertShipmentState(ShipmentKey);
        }

        private ShipmentState InsertShipmentState(Guid ShipmentKey) {

            var shipmentState = new ShipmentState { 
                Key = Guid.NewGuid(),
                ShipmentKey = ShipmentKey,
                Shipment_Status = PingYourPackage.Domain.Entities.ShipmentState.ShipmentStatus.Ordered,
         ...
         /*
        private bool IsShipmentStateInsertable(Guid shipmentKey, PingYourPackage.Domain.Entities.ShipmentState.ShipmentStatus status) {

            var shipmentStates = GetShipmentStates(shipmentKey);
            var latestState = (from state in shipmentStates
                               orderby state.Shipment_Status descending
                               select state).First();

            return status > latestState.Shipment_Status;
        }
        */
        private bool IsShipmentRemovable(Shipment shipment) {

            var latestStatus = (from shipmentState in shipment.ShipmentStates.ToList()
                                orderby shipmentState.Shipment_Status descending
                                select shipmentState).First();

            return latestStatus.Shipment_Status < PingYourPackage.Domain.Entities.ShipmentState.ShipmentStatus.InTransit;
        }
